...
<div class="text-xs-center">
    <v-menu v-model="menu" :close-on-content-click="false" :nudge-width="200" offset-y transition="slide-y-transition">
        <v-btn icon v-slot:activator="{ on }" dark>
        ...
        <v-card>
            <v-list>
                <v-list-tile avatar>
                    ...
                    <!-- <v-list-tile-action v-if="notifications.length > 0">
                        <v-tooltip bottom>
                            <v-btn v-slot:activator="{ on }" :class="fav ? 'red--text' : ''" icon @click="read()">
            ...
            <v-card-actions>
                <form action="/logout" method="post">
                    ...
                    <v-tooltip bottom>
                        <v-btn text v-slot:activator="{ on }" color="primary" icon class="mx-0" type="submit">
                ...
                <form action="/logoutOther" method="get">
                    ...
                    <v-tooltip bottom>
                        <v-btn text v-slot:activator="{ on }" color="orange" icon class="mx-0" type="submit">
